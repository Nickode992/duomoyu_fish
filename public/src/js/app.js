...
// Lightweight loading UI for ONNX model & WASM
...
// Intercept fetch for ONNX model and ORT wasm files to report progress
if (!window.__fetchPatchedForModel__) {
    ...
    window.fetch = async function(input, init) {
        ...
        try {
            ...
            // If this was the ONNX model, consider finishing after a brief delay
            if (/fish_doodle_classifier\.onnx/.test(url)) {
                setTimeout(() => finishModelLoader(), 600);
            }
            return resp;
        } catch (e) {
            // Keep loader visible but don't block
            return originalFetch(input, init);
        }
    };
}
