...
if (forceLocal) {
    ...
    BACKEND_URL = window.location.origin; // Workers 同域
} else {
    BACKEND_URL = isLocalhost ? 'http://localhost:8787' : window.location.origin;
}
...
// Normalize image URL to same-origin R2 proxy to avoid CORS when drawing to canvas (dev copy)
